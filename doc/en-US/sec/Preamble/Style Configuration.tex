...
When writing style code, please ensure that a display argument does not immediately follow any macro command, as this may lead to syntax parsing errors. You must insert a space before the \texttt{@} to prevent this issue, for example: \cs{par}\texttt{ @\meta{display argument}} .

\newpage Customizable style names and their available display arguments include:
...
\List{TOC Style}(\large Table of Contents){
    ...
    - \textbf{\texttt{TOCSection} : Section entry in the table of contents}.
        ...
        - - \texttt{@part} : Name of the part containing the section.
        - - \texttt{@group} , \texttt{@subgroup} ... : Top-level group, second-level group... containing the section.
        - - \texttt{@sectionpath} : Section path.
        - - \texttt{@sectionpage} : Page number where the section title is located.
...
\List{Cross-Reference Style}(\large Cross-Reference){
    ...
    - \textbf{\texttt{SectionLink} : Section Link}.
        ...
        - - \texttt{@targetsectionpage} : Page number where the target section title is located.
...
Before authoring style code, please review the \seclink{Style Code Preset Values}. \textcolor{orange}{It is highly recommended to first copy the specific portions you intend to modify into the preamble}, and then base your further adjustments upon them.
