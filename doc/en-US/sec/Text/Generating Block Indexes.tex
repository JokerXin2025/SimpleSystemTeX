...
The primary advantage of encapsulating content components within blocks is the ability to use the \cs{MakeBlockIndex} command anywhere in the document body to rapidly generate an index of all blocks of the same type within a specific group. Furthermore, block indices are equipped with cross-reference anchors, allowing you to \blclink{Add Block Index Link-U}(add block index links) elsewhere in the document to help readers navigate directly to the index.
...
The scope path serves as the unique identifier for cross-referencing a group index. Consequently, you \textcolor{red}{cannot generate two block indices with overlapping scopes}, even if they specify different lists of block types. If generating overlapping block indices is strictly necessary, you can use the following command to create a temporary block index that does not generate anchors:
