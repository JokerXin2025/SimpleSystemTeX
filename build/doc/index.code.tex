...
\cs_new_protected:Nn \_set_index:nn {
...
\cs_new_protected:Nn \_print_index_block:nn {
    \group_begin:
...
\cs_new_protected:Nn \_process_index_item:nnn {
    \group_begin:
...
\cs_new_protected:Nn \_generate_block_index:Nnnn {
    \group_begin:

    \clist_set:Nn \l_tmpa_clist {#3}

    % @index(...)group
    \_parse_path:nn {#2.groupkeep} {index}
    % @indexparameter
    ...
    \seq_map_inline:Nn \g_@@_block_register_seq {
        \clist_if_empty:NTF \l_tmpa_clist {
            \exp_args:Nnne \_process_index_item:nnn {#2} {##1} {\prop_item:Nn \g_@@_blocksectionpath_register_prop {##1}}
        } {
    ...
    \seq_map_inline:Nn \g_@@_nextgroup_seq {
        \_generate_block_index:Nnnn #1 {##1} {#3} {#4}
    }

    \group_end:
}
\cs_generate_variant:Nn \_generate_block_index:Nnnn {NnVV}
