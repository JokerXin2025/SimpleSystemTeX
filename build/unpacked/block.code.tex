...
\cs_new_protected:Nn \_set_block:nnnnn {
...
\cs_generate_variant:Nn \_set_block:nnnnn {neVVn}

% [parameter] block type name (only letters) , block's label (expanded) , block's display name , block's parameter , block's content
\cs_new_protected:Nn \_print_block:nnnnn {
    \group_begin:
    ...
    \int_compare:nT {\g_@@_Automatic_List_beta_int == 1} {
        \trans_auto_list:N \l_@@_blockcontent_tl
    }
...
\cs_generate_variant:Nn \_print_block:nnnnn {neVVn}
\cs_generate_variant:Nn \_print_block:nnnnn {nnVVn}

% [parameter] block type name (only letters)
\cs_new_protected:Nn \_declare_anonymous_block_type:n {
    ...
    \exp_args:Nc \NewDocumentCommand {#1} {d() d<> +m} {
        ...
        \_print_block:nnVVn {#1} {} \l_@@_block_arga_tl \l_@@_block_argb_tl {##3}
...
% [parameter] block type name (only letters)
\cs_new_protected:Nn \_declare_block_type:n {
    ...
    \exp_args:Nc \NewDocumentCommand {#1} {s m d() d<> +m} {
        ...
        \IfBooleanF {##1} {
            \_set_block:neVVn {#1} {##2} \l_@@_block_arga_tl \l_@@_block_argb_tl {##5}
        }

        \_print_block:neVVn {#1} {##2} \l_@@_block_arga_tl \l_@@_block_argb_tl {##5}
